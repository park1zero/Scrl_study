...
from shared_control_rl.torch_sac import checkpoint_metadata, load_actor
...
@torch.no_grad()
def rollout(env, model_path: str, deterministic: bool = True, curriculum_progress: float | None = None, seed: int | None = None, reset_options: dict | None = None):
    device = torch.device("cpu")
    actor = load_actor(model_path, device=device)
    ...
    while not (terminated or truncated):
        ...
        action_t = actor.act(obs_t, deterministic=deterministic)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate a minimal PyTorch SAC checkpoint.")
    ...
    parser.add_argument("--out", type=str, default="artifacts/eval_sac_policy.png")
    ...
    plot_history(history, env, title=f"PyTorch SAC policy (stack={history_stack})", out_path=Path(args.out))
