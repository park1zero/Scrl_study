...
def evaluate(ckpt: Path, driver_model: str = 'jbk', episodes: int = 24, summary_csv: Path = Path('artifacts/final_bc_policy_summary.csv'), preview_png: Path = Path('artifacts/final_bc_policy_preview.png'), compare_png: Path = Path('artifacts/final_bc_rollout_compare.png')) -> None:
    torch.set_num_threads(1)
    env = build_env(driver_model=driver_model, randomize=True)
    bc_policy = BCPolicy(ckpt)
    policies = {'driver_hold': driver_hold_policy, 'heuristic': heuristic_authority_policy, 'bc_warmstart': bc_policy}
    summaries = []
    rollout_bank = {}
    same_seed = 1003
    ...
    plt.ylim(0.0, 1.05)
    plt.title(f'BC warm-start policy comparison ({driver_model})')
    plt.tight_layout()
    plt.legend()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', type=str, default='artifacts/jbk_bc_actor.pt')
    parser.add_argument('--driver-model', type=str, default='jbk')
    parser.add_argument('--episodes', type=int, default=24)
    parser.add_argument('--summary-csv', type=str, default='artifacts/final_bc_policy_summary.csv')
    parser.add_argument('--preview-png', type=str, default='artifacts/final_bc_policy_preview.png')
    parser.add_argument('--compare-png', type=str, default='artifacts/final_bc_rollout_compare.png')
    args = parser.parse_args()
    evaluate(Path(args.ckpt), driver_model=args.driver_model, episodes=args.episodes, summary_csv=Path(args.summary_csv), preview_png=Path(args.preview_png), compare_png=Path(args.compare_png))
