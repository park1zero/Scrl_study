...
from driver import DriverParams, DriverState, initialize_driver_state, step_driver
...
@dataclass
class EnvParams:
    """Reward, action, and termination parameters for the RL environment.

    No safety filter is used in this step. The RL action updates lambda directly:

        lambda_{k+1} = clip(lambda_k + lambda_rate_max * dt * a_k, 0, 1)

    with action a_k in [-1, 1].
    """
...
class SharedControlEnv:
    """Minimal RL environment for authority allocation without a safety filter."""
    ...
    def __init__(
        ...
        self.base_driver_params = driver_params or DriverParams()
        ...
        self.driver_state: DriverState = initialize_driver_state(self.driver_params, self.vehicle_params.dt)
        self.step_count = 0
    ...
    def reset(self, seed: Optional[int] = None) -> Tuple[np.ndarray, Dict[str, float]]:
        ...
        self.driver_params = DriverParams(**self.base_driver_params.__dict__)
        ...
        self.driver_state = initialize_driver_state(self.driver_params, self.vehicle_params.dt)
        self.step_count = 0
    ...
    def step(self, action: float) -> Tuple[np.ndarray, float, bool, bool, Dict[str, float]]:
        ...
        lam_next = clip(
            lam_prev + self.env_params.lambda_rate_max * self.vehicle_params.dt * action,
            0.0,
            1.0,
        )
